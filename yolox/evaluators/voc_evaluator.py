...
class VOCEvaluator:
    ...
    def evaluate(
        ...
        for cur_iter, (imgs, therm, _, info_imgs, ids) in enumerate(progress_bar(self.dataloader)):
            with torch.no_grad():
                imgs = imgs.type(tensor_type)
                therm = therm.type(tensor_type)
                ...
                # print("CONF_tensor", outputs[:, :, 4])
                # print("PERSON_tensor", outputs[:, :, 5])
                outputs = postprocess(
                    outputs, self.num_classes, self.confthre, self.nmsthre
                )
                # print("CONF_tensor", outputs)
                if is_time_record:
