...
class Trainer:
    ...
    def train_one_iter(self):
        ...
        inps, inps_t, targets = self.prefetcher.next()
        # img = tensor_to_np(inps)
        # cv2.imwrite('/home/cnu228/Documents/cyx/YOLOX-train-your-data/result_cache/stn_out/result2.jpg', img)
        # print(targets.shape)
        ps = inps.to(self.data_type)
        targets = targets.to(self.data_type)
        targets.requires_grad = False
        data_end_time = time.time()

        outputs = self.model(inps, inps_t, targets)
        ...
        if self.amp_training:
            ...
            loss.backward()
        self.optimizer.step()
        self.tblogger.add_scalar("loss", loss, self.epoch + 1)
    ...
    def after_epoch(self):
        ...
        self.save_ckpt(ckpt_name="mdnet_latest")
    ...
    def resume_train(self, model):
        if self.args.resume:
            ...
            if self.args.ckpt is None:
                ckpt_file = os.path.join(self.file_name, "mdnet_latest" + "_ckpt.pth.tar")
            else:
